...
def _queue_file():
    return persona_data_dir() / "content_queue.json"

def _report_file():
    return persona_data_dir() / "daily_report.md"
...
def generate_report() -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*🤖 {get_persona().get('report_footer', 'Generated by Bot')}*")
...
def run_daily_report() -> str:
    ...
    try:
        ...
        with open(rf, "w") as f:
            f.write(report)
        log.info("Daily report saved to %s", rf)
    except Exception as exc:
